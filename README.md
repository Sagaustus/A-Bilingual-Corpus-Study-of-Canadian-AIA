...
- **4 LLM interpretation tables** — automation justification, risk/rights impact, bilingual divergence, safeguard compliance (30 rows each)
- **1 thematic patterns table** — 22 cross-submission themes identified by LLM
- **1 run log table** — ETL execution tracking
- **1 view** — `v_datasets_flat` denormalized view of the dataset layer

### LLM Semantic Interpretations

The interpretation tables are generated by sending respondent answers to Llama 3.3 70B (via IONOS) with structured prompts. Run the ETL:
